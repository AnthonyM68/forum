...
if ($result['section'] === "profile") { ?>
    ...
    $restor = $result['restor'];
    if ($user) { ?>
        <div class="uk-animation-fade uk-container">
            <div class="uk-grid-margin uk-grid uk-grid-stack" uk-grid>
                <div class="uk-width-1-1@m">
                    <h1 class="pridi-regular uk-animation-slide-top">Votre Profil <?= $user->getUsername() ?></h1>
                    <form action="<?php echo $restor ? $restor = 'index.php?ctrl=security&action=restorAccount&id='. $user->getId() .'&token=' . $result['token'] : $restor = 'index.php?ctrl=security&action=modifyAccount' ?>" method="post" uk-grid>
                            ...
                            <div class="uk-margin">
                                ...
                                <div class="uk-grid-small uk-flex uk-flex-between" uk-grid>
                                    <p class="uk-text-center" uk-margin>
                                        <button type="submit" class="uk-text-center uk-button uk-button-default">Restaurer mon compte</button>
                                    </p>
                                </div>
                            <?php
                                    } else { ?>
